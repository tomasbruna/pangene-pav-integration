...
my $YLIMIT = 30000; # default Y-axis limit
getopts('hagselxnp:m:I:A:B:P:S:R:y:', \%opts);
...
# Add the parameter check:
if(defined($opts{'y'}) && $opts{'y'} > 0) {
    $YLIMIT = $opts{'y'};
}
...
# Replace the barplot section with:

# Replace the barplot sections with:

pdf(file="$shell_output_pdf");
bars = barplot(shell\$V1,xlab='number of genomes in clusters (occupancy)',ylab='number of gene clusters',
    main='',names.arg=1:$n_of_taxa,col=colors,ylim=c(0,$YLIMIT));
      
# Add numbers on top of bars, excluding shell class if more than 20 bars
if ($n_of_taxa > 20) {
    # For cloud region
    text(x=bars[1:$cloudpos], y=shell\$V1[1:$cloudpos], 
         labels=shell\$V1[1:$cloudpos], pos=3, cex=0.8)
    # For soft core and core regions
    text(x=bars[$softcorepos:$n_of_taxa], y=shell\$V1[$softcorepos:$n_of_taxa], 
         labels=shell\$V1[$softcorepos:$n_of_taxa], pos=3, cex=0.8)
} else {
    text(x=bars, y=shell\$V1, labels=shell\$V1, pos=3, cex=0.8)
}
...
png(file="$shell_output_png", width=8.5, height=6, units="in", res=600);
bars = barplot(shell\$V1,xlab='number of genomes in clusters (occupancy)',ylab='number of gene clusters',
